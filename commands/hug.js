...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option
        .setName('target')
        .setDescription('Select a member to hug')
        .setRequired(true),
    ),
  async execute(interaction) {
    const term = 'anime hug'
    await interaction.reply({
      content: null,
      embeds: [
        {
          title: `${await interaction.member.user
            .username} hugged ${await interaction.options.getUser('target')
            .username} !`,
          color: null,
          image: {
  ...
  async executeManual(message, content) {
    if (content[1]) {
      try {
        try {
          ...
          await message.reply({
            ...
            embeds: [
              {
                title: `${message.author.username} hugged ${targetuser.username} !`,
                color: null,
                image: {
                  url: await gifapi(term),
                },
      ...
      await message.reply({
        ...
        embeds: [
          {
            title: `${message.author.username} hugged ${message.author.username} !`,
            color: null,
            image: {
              url: await gifapi(term),
            },
